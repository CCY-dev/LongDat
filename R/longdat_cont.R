...
longdat_cont <- function(input, test_var, value_column, factor_column, non_factors,
  ...
  for (i in 1:N) { # loop through all variables
    ...
    for (k in 1:length(sel_fac_ini[[i]])) {# Loop through all sel_fac
      fac <- c()
      for (m in 1:length(unique(subdata$Individual))) { # Loop through all individuals
  ...
  p_poho <- matrix(nrow = length(variables), ncol = 1)
  assoc <- matrix(nrow = length(variables), ncol = 1)
  ...
  # First determine the variables with post-hoc q < 0.05,
  # then test whether inverse model q < 0.1 (confounded) or q > 0.1 (deconfounded)
  con <- c()
  con <- (p_poho < posthoc_p)
  con_type <- c()
  ...
  print("Confounders.txt is in your directory.")
  ...
  # Model q < 0.1 and p_poho < 0.05 and association < 0 is depleted
  # Model q < 0.1 and p_poho < 0.05 and association > 0 is enriched
  ...
  sig_det <- function(x) {
    ifelse(test = result_table$Correlation[x] == "NS",
           yes = "Insignificant", no = "OK")
  }

  result_table$Signal <- NA
  for (i in 1:length(variables)) {
    result_table$Signal[i] <- sig_det(i)
  }
